...
КРИТИЧНО ДЛЯ beauty.weaknesses ("Особенности"):
- Это НЕ список недостатков и НЕ рекомендации что исправить.
- Это наблюдения о структурных чертах, которые формируют характер восприятия.
- Тон: нейтральный или слегка позитивный — "мягкая линия челюсти создаёт впечатление открытости", а не "слабая челюсть".
- ЗАПРЕЩЕНО советовать что изменить, как улучшить, к кому обратиться.
- ЗАПРЕЩЕНО упоминать стрижку, бороду, макияж, одежду — только черты лица.
- Каждый пункт заканчивается тем, как эта черта считывается окружающими.
...
- Источник данных: grooming_signals (tension_zones, skin_signals, skin_level, grooming_level)
- Структура: verdict_quote, current_impression, strengths, improvements, style_archetype, style_tags
- Этот блок — про состояние, а не про внешность. Читаем напряжение, усталость, внутренний ресурс по лицу.
- РАЗРЕШЕНО: упоминать зоны усталости и напряжения (лоб, веки, челюсть), сигналы стресса, уровень энергии
- ЗАПРЕЩЕНО: одежда, стиль одежды, стрижка, причёска, борода, советы по уходу за кожей, косметика
- ЗАПРЕЩЕНО: описывать недостатки кожи (поры, пятна, тон) — мы не дерматологи и не стилисты
- style_archetype — это не модный тип, а энергетический образ (например: "тихая сила", "концентрированное внимание")
- style_tags — ключевые слова состояния, не одежды (например: "собранность", "скрытое напряжение")
- improvements — рекомендации только про восстановление ресурса (сон, режим, снятие напряжения), не про внешность
- accessories — игнорируй полностью, не упоминай
